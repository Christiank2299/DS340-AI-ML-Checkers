...
def gameCount( data='data', crsr=crsr):
    ...
    for i in range(len(games)):
        sumgame += games[i][0]
        sumwins += wins[i][0]
    print("Different Moves:", len(games),"\nTotal n:", sumgame, "\nTotal q:", sumwins, "\nAverage reward per visit: %.4f" %(sumwins/(sumgame + 0.00000000001)), "\nVisits per node: %2.2f" %(sumgame / ((len(games)) + 0.000000000001) ))
    #added the 0.00000000001 at the end of the division to avoid the division by 0 error 
